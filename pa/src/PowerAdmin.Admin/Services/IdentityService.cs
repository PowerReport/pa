...
namespace PowerAdmin.Admin.Services
{
    public class IdentityService : IIdentityService
    {
        ...
        public async Task<CreateUserCase.Response> CreateUser(CreateUserCase.Request request)
        {
            var user = await userService.Create(request.UserName, request.Password, request.ConfirmPassword, request.PhoneNumber, request.Email, request.TwoFactorEnabled);
