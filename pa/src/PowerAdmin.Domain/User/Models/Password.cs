...
public class Password {
  ...
  operator string(Password password) => password.ToString();
  ...
  /// <param name="password1"></param>
  /// <returns></returns>
  public static bool operator !=(Password? password,
  ...
  private static void ValidatePassword(string str) {
    ...
    if (!IncludeDigit(str) || !IncludeUpperChar(str) ||
        !IncludeSpecialChar(str)) {
      throw Oops.Bah(
          "不正确的密码格式, 需要大小写字母、数字和特殊字符三种或以上: {0}!",
          str);
    }
  }
  ...
  /// <returns></returns>
  private static bool
  IncludeUpperChar(string str) => str.Any(c => char.IsUpper(c));
  ...
  /// <returns></returns>
  private static bool
  IncludeSpecialChar(string str) => str.Any(c => !char.IsLetterOrDigit(c));
}
