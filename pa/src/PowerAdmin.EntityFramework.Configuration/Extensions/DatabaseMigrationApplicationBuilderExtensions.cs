...
public static class DatabaseMigrationApplicationBuilderExtensions {
  ...
  EnsureDatabasesMigrated<TUserIdentityDbContext>(this IApplicationBuilder app)
  ...
  EnsureSeedIdentityData(this IApplicationBuilder app) {
    ...
    var identityData =
        App.Configuration.GetSection(nameof(IdentityData)).Get<IdentityData>();
      ...
      foreach (var role in identityData.Roles) {
        ...
        if (result.Succeeded) {
          foreach (var claim in role.Claims) {
            await roleManager.AddClaimAsync(userIdentityRole,
      ...
      foreach (var user in identityData.Users) {
        ...
        if (result.Succeeded) {
          foreach (var claim in user.Claims) {
            await userManager.AddClaimAsync(userIdentity,
                                            new Claim(claim.Type, claim.Value));
          }
