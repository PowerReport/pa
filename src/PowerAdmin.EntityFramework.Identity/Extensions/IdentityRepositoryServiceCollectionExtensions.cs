...
namespace PowerAdmin.EntityFramework.Identity.Extensions
{
    ...
    {
        public static IServiceCollection AddIdentityRepository<TUserIdentityDbContext>(this IServiceCollection services)
            where TUserIdentityDbContext : DbContext
        {
            // 添加用户身份管理相关服务
            services.AddIdentity<UserIdentity, UserIdentityRole>(options => App.Configuration.GetSection(nameof(IdentityOptions)).Bind(options))
